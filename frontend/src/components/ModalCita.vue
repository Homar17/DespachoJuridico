    ...
    <div class="bg-despacho-oscuro text-despacho-fondo w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-despacho-fondo/10">
      ...
      <form @submit.prevent="enviarFormulario" class="p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
          ...
          <div class="relative">
            ...
            <select 
              ...
              class="w-full bg-white/5 border border-despacho-fondo/20 text-despacho-fondo rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-despacho-rojo transition-all appearance-none disabled:opacity-50 disabled:cursor-not-allowed"
              required
            >
              <option value="" disabled class="bg-despacho-oscuro">
                {{ cargandoHoras ? 'Buscando disponibilidad...' : (horasDisponibles.length === 0 ? 'Sin horarios disponibles' : 'Selecciona una hora') }}
              </option>
              <option 
            ...
            <div class="pointer-events-none absolute bottom-0 right-0 flex items-center px-4 pb-4 text-despacho-fondo/50">
              <svg v-if="!cargandoHoras" class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
              <svg v-else class="h-4 w-4 animate-spin text-despacho-rojo" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div class="pt-2 flex gap-4">
          ...
          >
          {{ cargando ? 'Redirigiendo al pago...' : 'Pagar Asesoría ($200 MXN)' }}
          </button>
        </div>
      </form>
    </div>
...
const fechaSeleccionada = ref(fechaMin)
...
const obtenerHorasDelBackend = async (fecha) => {
if (!fecha) return;
...
try {
  ...
  const response = await axios.get(`http://localhost:5039/api/citas/disponibilidad/${fecha}`, {
...
if (diaDeLaSemana === 0 || diaDeLaSemana === 6) {
  ...
  fechaSeleccionada.value = fechaMin;
} else {
  obtenerHorasDelBackend(fechaSeleccionada.value);
}
}
...
if (diaSemana === 0 || diaSemana === 6) {
    errorMsg.value = "La fecha seleccionada es inhábil."
    return
}
